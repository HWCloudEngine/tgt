...
static int bs_socket_init(hijacker_volume_t* volume, char* host, short port)
{
    ...
    int buf_size = 8 * 1024 * 1024 * 1024L;
    ...
    while (0 != nleft) {
        ...
        if (-1 == nwrite) {
            if(errno == EAGAIN || errno == EINTR){
                eprintf("failed to socket write busy err:%d \n", errno);
                continue;
            }
            eprintf("failed to socket write err:%d nwrite:%ld \n", 
                    errno, nwrite);
...
int _eventfd_write(int eventfd){
    ...
    //eprintf("eventfd write ok ret=%d \n", ret);
    return 0;
}
...
void eventfd_callback(int fd, int events, void* data)
{
    ...
    //eprintf("tid:%ld xxxx eventfd_callback \n", pthread_self());
    _eventfd_read(fd);
    ...
    //eprintf("tid:%ld xxxx eventfd_callback end \n", pthread_self());
}
...
void socket_write_callback(int fd, int events, void* data)
{
    ...
    if(list_empty(&volume->sending_list)){
        eprintf("xxxx socket_write_callback send queue empty \n");
        return;
    }

    eprintf("xxxx socket_write_callback \n");
    ...
    eprintf("xxxx socket_write_callback end \n");

    if(list_empty(&volume->sending_list)){
        int ret = tgt_event_modify(volume->jwriter_sock, EPOLLIN | EPOLLET);
        if(ret < 0){
            eprintf("xxxx socket_write_callback modify epolin failed \n");
            return;
        }
    }
}
...
void socket_read_callback(int fd, int events, void* data)
{
    int ret = 0;
    hijacker_volume_t* volume = (hijacker_volume_t*)data;
    eprintf("xxxx socket_read_callback \n");
    ...
    while(true){
        ...
        if(ret != sizeof(reply_head)){
            eprintf("xxxx bs_socket_recv read failed ret:%d want:%ld \n",
        ...
        if(reply->len > 0){
            ...
            if(ret != reply->len){
                eprintf("xxxx bs_socket_recv read failed ret:%d want:%d \n",
                    ret, reply->len);
                break;
            }
        }
    ...
    ret = tgt_event_add(volume->pending_eventfd, EPOLLIN,
                        eventfd_callback, volume);
    if(ret){
        eprintf("failed to add pending read event, errno:%d \n", errno);
    }

    eprintf("xxxx socket_read_callback ok \n");
}

void socket_callback(int fd, int events, void*data)
{
    //eprintf("xxxx socketfd_callback \n");
...
static int bs_hijacker_cmd_submit(struct scsi_cmd* cmd){
    ...
    eprintf("tid:%ld cmd submit cmd cdb[0]=%d \n", pthread_self(),cmd->scb[0]);
